...
def inquirys(request):
    ...
    if form.is_valid():
        ...
        newquery = UserQuery(name=name, surname=surname, email=email, phone=phone,
                             username=username, course_name=course_name, course_email=course_email)
        newquery.save()
        messages.success(
            request, "Sorgu ugurla elave edildi. Qisa zamanda sizinle elaqe saxlanilacaq")
...
def teacherquery(request):
    ...
    if form.is_valid():
        ...
        newquery = TutorQuery(teachername=name, teachersurname=surname, email=email,
                              teacherphone=phone, experience=experience, study_field=study_field)
        newquery.save()
        messages.success(
            request, "Sorgu ugurla elave edildi. Qisa zamanda sizinle elaqe saxlanilacaq")
        return redirect("index")
